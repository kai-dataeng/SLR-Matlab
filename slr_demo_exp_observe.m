...
load('D:\WORK\Matlab II\speed limit recognition\3-digit recognition - nn\slr_nn2');
load slr_init_data;
...
%     tic;
    str2 = ' ';
    result = 0;
    img = imread(file_img(rp(cnt)).name);
...
%             disp(['searching on scale: ',num2str(scale),' completed']);
        ...
        if num_obj<2
            ...
            if obj_count>1            
                ...
                if(result)
                    str2 = [str,'km/j'];
                    str = [str,'_'];
                    if(strncmp(str,file_img(rp(cnt)).name,3))
                        TA = TA+1;
                        str2 = [' => ',str2];
                    else
                        FA = FA+1;
    ...
    disp([num2str(cnt),') TS: ',num2str(TS),...
