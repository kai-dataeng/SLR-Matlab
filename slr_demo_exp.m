...
cur_dir = pwd;
load('D:\WORK\Matlab II\speed limit recognition\1-signboard detection - color\sb_color_boundary');
load('D:\WORK\Matlab II\speed limit recognition\2-circle detection - tm\circle_filter');
load('D:\WORK\Matlab II\speed limit recognition\3-digit recognition - nn\slr_nn');
load slr_init_data;
...
%     figure(f1),subplot(1,3,1),imshow(img);
...
%     drawnow;
    ...
    T1 = max(toc,T1);
    Ttemp = toc;
%     figure(f1),subplot(1,3,2),imshow(map_img);
...
%     figure(f1),subplot(1,3,2),imshow(BW);
...
%         drawnow; 
        ...
        Ttemp = toc;
        if num_obj<2
%             figure(f1),subplot(1,3,2),imshow([]);
...
%                     figure(f1),subplot(1,3,3),imshow(temp_map); title(['detected digit']);                
...
%                 figure(f1),subplot(1,3,3),imshow(digit_img);
...
%                 drawnow; 
                ...
                if(result)
                    str2 = [str,'km/j'];
                    str = [str,'_'];
                    if(strncmp(str,file_img(cnt).name,3))
                        TA = TA+1;
                        str2 = [' => ',str2];
                    else
                        FA = FA+1;
...
%         drawnow;   
    ...
    disp([num2str(cnt),') Ctime: ',num2str(cur_time,3),' Ttime: ',num2str(total_time,3),' TS: ',num2str(TS),...
         ' TA: ',num2str(TA),' FA: ',num2str(FA),' FR: ',num2str(FR),' TUD: ',num2str(TUD),str2]);
%     clf;
end
close all;
disp([' ']);
...
disp(['TAR: ',num2str(TA,2),' FAR: ',num2str(FA,2),' FR: ',num2str(FR,2),' TUDR: ',num2str(TUD,2)]);
disp(['T1: ',num2str(T1,2),' T2: ',num2str(T2,2),' T3: ',num2str(T3,2),' T4: ',num2str(T4,2),' T5: ',num2str(T5,2)]);
disp(['completed...']);
cd(cur_dir);
