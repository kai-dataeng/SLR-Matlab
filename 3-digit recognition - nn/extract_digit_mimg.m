...
%     BW = imopen(BW,SE);    
%     BW = bwmorph(BW,'open');
    uw_area = round(row*col*1e-2);
    BW = bwareaopen(BW,uw_area);
    
    figure(f1),subplot(1,2,2),imshow(BW);
    title(['remove unwanted objects']); drawnow; pause(1);
    ...
    if num_obj<2
        figure(f1),subplot(1,2,2),imshow([]);
        ...
        if obj_count>1            
            ...
            temp = []; temp2 = []; st = 1;
            for i=1:length(ind)
                temp = [temp digit_data(ind(i)).data];
...
close all;
cd(cur_dir);
save slr_data slr_sdata slr_width slr_height;
disp(['completed...']);
