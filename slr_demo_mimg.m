...
cur_dir = pwd;
load('D:\WORK\work - Matlab II\speed limit recognition\1-signboard detection - color\sb_color_boundary');
load('D:\WORK\work - Matlab II\speed limit recognition\2-circle detection - tm\circle_filter');
load('D:\WORK\work - Matlab II\speed limit recognition\3-digit recognition - nn\slr_nn');
load slr_init_data;
...
%             disp(['searching on scale: ',num2str(scale),' completed']);
    ...
    if result
        disp(['RESULT for image ',file_img(cnt).name,' : ',str,' km/h']);
    else
        disp(['RESULT for image ',file_img(cnt).name,' : ------------']);
    end
    pause(disp_lasttime);
    clf;
end
